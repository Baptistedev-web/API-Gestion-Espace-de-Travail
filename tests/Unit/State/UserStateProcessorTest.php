...
#[ CoversClass(UserStateProcessor::class) ]
class UserStateProcessorTest extends TestCase
{
    ...
    public function testProcessThrowsExceptionWhenUserNotFound(): void
    {
        ...
        $userRepository
            ...
            ->method('find')
            ->with(1)
            ->willReturn(null); // Simule un utilisateur non trouvé
        ...
        $this->processor->process($user, $this->createMock(Operation::class));
    }

    public function testProcessUpdatesUserWithNewPassword(): void
    {
        $user = new User();
        $reflection = new \ReflectionClass($user);
        $property = $reflection->getProperty('id');
